...
Olayout CSSは、俺流のレイアウト用CSSフレームワークです。CSSファイルに移動せず、マークアップしながらカスタムデータ属性でWebページのレイアウトがをすることにフォーカスしています。

ユーティリティファーストなコーディングを行いたい場合は、確実に [Tailwind CSS](https://tailwindcss.com/) などのフレームワークの方が便利です。
...
<Tabs>
  <TabItem label="CDN">
    ...
    1. CDNのOlayout CSSを追加<br />`head`要素内に`link`要素を用いてCDN経由のOlayout CSSを読み込んでください。
       ...
       <body>
         <h1 data-color="main">Olayout CSS</h1>
       </body>
       </html>
       ```
